...
from backend.scrapes.ice_python.symbols.futures.gas import (
...
# Fields pulled per symbol. Long-format storage with PK
# (trade_date, symbol, data_type); illiquid back-month contracts may publish
# Settle but no Volume/Open/etc., so absent rows for a (symbol, field) pair
# are expected and not treated as failures.
#
# Source of truth: backend/scrapes/ice_python/fields/. Edit the curated set
# in fields/presets.py (FUTURE_CONTRACTS_FIELDS) rather than this alias. To
# inspect the full 33-field ICE catalog for ad-hoc probing, use
# backend/scrapes/ice_python/future_contracts/test_future_contracts.py.
DEFAULT_FIELDS: list[str] = FUTURE_CONTRACTS_FIELDS

# Minimum fraction of expected symbols that must return data for a run to be
# considered healthy. Symbol-level (any field counts); per-field sparsity is
# captured separately in the run metadata.
...
def main(
    ...
    data_types = data_types or list(DEFAULT_FIELDS)
    ...
    if specific_products:
        ...
        if not products:
            raise ValueError(
                f"specific_products={specific_products} matched no entries "
                f"in the loaded registries."
            )
    ...
    try:
        ...
        logger.info(f"Fields ({len(data_types)}): {data_types}")
...
if __name__ == "__main__":
    # Run for PMI K26-IUS only (PJM PJM-MISO Interface, May 2026 strip).
    main(
        specific_products=["PMI"],
        specific_strips=["K"],
