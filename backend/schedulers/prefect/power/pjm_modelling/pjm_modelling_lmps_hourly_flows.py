...
MART_NAME = "pjm_lmps_hourly"
SCHEMA = "pjm_cleaned_v2"


def run_dbt(select: str) -> None:
    """Run dbt models by selection syntax (e.g. '+pjm_modelling_lmps_hourly')."""
...
@flow(name="PJM Modelling LMPs Hourly")
def pjm_modelling_lmps_hourly():
    """Refresh the pjm_modelling_lmps_hourly view and sync to Azure Blob parquet."""
    run = pipeline_run_logger.PipelineRunLogger(
        pipeline_name=MART_NAME, source="power",
    )
    run.start()
    try:
        # ────── 1. Rebuild the view (and any ephemeral upstream logic) ──────
        run_dbt(f"+{MART_NAME}")
