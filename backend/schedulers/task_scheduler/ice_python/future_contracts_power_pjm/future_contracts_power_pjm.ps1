# Registers the ICE future contracts (PJM power region) Task Scheduler job.
...
$moduleName  = "backend.orchestration.ice_python.future_contracts.future_contracts_power_pjm"

$cmdArgs = "/k `"call `"$condaPath`" $condaEnv && cd /d `"$repoRoot`" && python -m $moduleName`""
...
Register-ScheduledTask `
    -TaskName "Future Contracts Power PJM" `
    -Action $action `
    -Trigger $trigger `
    -Settings $settings `
