"""Task Scheduler entry point for the gas-futures contract-dates scrape.

Wraps `contract_dates_v1.main` with a weekday gate and an ICE-transient
retry. Sibling of `contract_dates_power_ercot` / `contract_dates_power_pjm`
— each fires the same parameterized scrape against a different product
registry.

Usage (Task Scheduler):
    python -m backend.orchestration.ice_python.contract_dates.contract_dates_gas
"""
...
PIPELINE_NAME = "runner_future_contracts_gas_contract_dates"
PRODUCT_LABEL = "gas futures"
