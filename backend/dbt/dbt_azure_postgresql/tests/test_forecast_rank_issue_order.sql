WITH ALL_VINTAGES AS (
    ...
    SELECT
        ...
        forecast_rank
    FROM (
        SELECT DISTINCT forecast_date, source, region, forecast_execution_datetime, forecast_rank
    ...
    SELECT
        ...
        forecast_rank
    FROM (
        SELECT DISTINCT forecast_date, hub, forecast_execution_datetime, forecast_rank
    ...
    SELECT
        ...
        forecast_rank
    FROM (
        SELECT DISTINCT forecast_date, forecast_execution_datetime, forecast_rank
    ...
    SELECT
        'staging_v1_pjm_solar_forecast_hourly' AS model_name,
        forecast_date,
        NULL::TEXT AS partition_dim_1,
        NULL::TEXT AS partition_dim_2,
        forecast_execution_datetime AS issue_timestamp,
    ...
    FROM (
        SELECT DISTINCT forecast_date, forecast_execution_datetime, forecast_rank
        FROM {{ ref('staging_v1_pjm_solar_forecast_hourly') }}
    ) d

    UNION ALL

    SELECT
        'staging_v1_pjm_wind_forecast_hourly' AS model_name,
        forecast_date,
        NULL::TEXT AS partition_dim_1,
        NULL::TEXT AS partition_dim_2,
        forecast_execution_datetime AS issue_timestamp,
    ...
    FROM (
        SELECT DISTINCT forecast_date, forecast_execution_datetime, forecast_rank
        FROM {{ ref('staging_v1_pjm_wind_forecast_hourly') }}
    ) d
