"""Pull ice_python.pmi_k26_ius_wide via the committed compiled SQL.

Reads sql/pmi_k26_ius_wide.sql (rendered by dbt and committed
alongside this module), executes it as a SELECT against Azure
Postgres, times the pull, and returns a DataFrame.

Usage:
    python -m backend.views.ice_python.pmi_k26_ius_wide
"""
...
SQL_PATH = Path(__file__).parent / "sql" / "pmi_k26_ius_wide.sql"


def main(sql_path: Path = SQL_PATH) -> pd.DataFrame:
    logger = init_logging(name="pmi_k26_ius_wide", log_to_file=False)
    ...
    logger.info(f"Pulled {len(df):,} rows from {sql_path.name}")
    return df
