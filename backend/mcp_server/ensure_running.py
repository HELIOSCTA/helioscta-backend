...
Used by the PreToolUse health-check hook (and runnable directly). The
contract:

  1. Kill any process listening on PORT (yours, a stale uvicorn,
     anything — no diff between "healthy MCP" and "stale process").
  2. Spawn a detached uvicorn with stdout/stderr → backend/mcp_server/logs/server.log.
  3. Poll /openapi.json until healthy or timeout.
  4. Exit 0 on healthy, 1 on timeout.

This guarantees each invocation runs against the code on disk — edits
to data layer / view builders / formatters are picked up automatically.

Exit codes:
    0  = MCP server is up and healthy.
    1  = Failed to come up within READY_TIMEOUT_SEC. See log.
