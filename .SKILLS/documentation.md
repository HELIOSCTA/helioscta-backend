# Documentation Skill

How to maintain and extend the Docusaurus documentation site in `documentation/`.

## Stack

- **Framework:** Docusaurus 3 (React-based, Node.js)
- **Config:** `documentation/docusaurus.config.js`
- **Sidebar:** `documentation/sidebars.js`
- **Content:** `documentation/docs/` directory (Markdown)
- **Custom CSS:** `documentation/src/css/custom.css`
- **CI/CD:** `.github/workflows/docs.yml` deploys to GitHub Pages on push to `main`

## Local Development

```bash
cd documentation
npm install     # first time only
npm start       # live preview at localhost:3000
npm run build   # production build to documentation/build/
npm run serve   # serve production build locally
```

## Theme

- Dark mode is the default (`colorMode.defaultMode: "dark"`)
- True black background (`#000`) via CSS custom properties in `documentation/src/css/custom.css`
- Full-width content layout (no max-width container)
- Active sidebar items highlighted in amber (`#ffc245`)
- High-contrast text (`rgba(255,255,255,0.92)` on `#000`)

## Sidebar Navigation Rules

- Labels should be concise, 2-4 words (e.g., "DA LMPs" not "PJM Day-Ahead Hourly LMPs")
- Defined in `documentation/sidebars.js` using explicit doc IDs (not autogenerated)
- Group related pages under categories; keep hierarchy max 3 levels deep
- Use "Scrapes" and "dbt Views" as standard subsection names
- **Standard domain category structure:** Every domain category must use three flat doc items — Overview, Scrapes, dbt Views — matching the pattern below. Do NOT nest scrape pages as sub-categories; consolidate all scrape cards onto a single `<domain>-scrapes.md` page.

```js
{
  type: "category",
  label: "<Domain>",
  collapsed: true,
  items: [
    { type: "doc", id: "domains/<domain>/overview", label: "Overview" },
    { type: "doc", id: "domains/<domain>/scrapes/<domain>-scrapes", label: "Scrapes" },
    { type: "doc", id: "domains/<domain>/dbt-views/<domain>-cleaned", label: "dbt Views" },
  ],
}
```

## Content Conventions

- Every domain overview opens with a 1-2 sentence summary
- Follow with `**Use this page** to...` to orient the reader
- Add "Next steps" blocks at page bottoms linking to related resources
- Admonitions use `:::note` / `:::tip` / `:::warning` syntax (NOT `!!!`)
- The home page (`documentation/docs/README.md`) has `slug: /` front matter to serve as the root
- Terminology should be consistent across all pages

## Page Templates

### Domain Overview

```markdown
# Domain Name

One-two sentence summary of what data this domain covers and why it matters.

**Use this page** to find scrape scripts, raw tables, and dbt views for [domain].

## Data Source
## Scrape Inventory
## dbt Views
## Known Caveats
## Owner
```

### Scrape Card

```markdown
## Scrape Card

| Field | Value |
|-------|-------|
| **Script** | path |
| **Source** | API name |
| **Target Table** | schema.table |
| **Trigger** | Scheduled / Event-Driven |
| **Cadence** | frequency |
| **Freshness** | lag description |

## Business Purpose
## Data Captured
## Primary Key
## Downstream
## Known Caveats
```

### dbt Mart View Card (Docusaurus)

Every mart view listed on a Docusaurus dbt-views page (`documentation/docs/domains/<domain>/dbt-views/`) must use this table format:

```markdown
### <mart_name>

| Field | Value |
|-------|-------|
| **Business Definition** | One-sentence description |
| **Grain** | One row per ... |
| **Primary Keys** | `col_a`, `col_b`, `col_c` |
| **Upstream** | `staging_v1_<model>` |
| **Use Cases** | Comma-separated use cases |
| **Refresh** | View -- refreshes on query |
| **SQL** | [GitHub](https://github.com/helioscta/helioscta-backend/blob/main/backend/dbt/dbt_azure_postgresql/models/<domain>/<domain>_cleaned/.docs/<mart_name>.sql) |
```

**Row order is strict** — do not reorder or omit rows. The `**SQL**` row is required and must link to the corresponding `.docs/<mart_name>.sql` file on GitHub.

### dbt Mart Documentation (`.docs/` folder)

Each dbt domain has a `.docs/` folder at the `<domain>_cleaned` level containing SQL reference files and a README:

```
models/<domain>/<domain>_cleaned/.docs/
  README.md              — mart-specific documentation
  <mart_name>.sql        — one file per mart view (e.g., pjm_lmps_hourly.sql)
```

**Why `.docs/` instead of `docs/`?** dbt compiles every `.sql` file under `models/` as a model. Placing documentation SQL files in a dot-prefixed folder and adding `.docs/` to `.dbtignore` prevents dbt from treating them as models (which caused name collisions).

**`.dbtignore` entry** (already configured):
```
**/.docs/
.docs/
```

#### Reference SQL Files

Each mart view gets its own `.sql` file named identically to the mart model. The file contains a `SELECT` listing every column with `FROM <schema>.<view_name>`:

```sql
select
  datetime
  ,date
  ,hour_ending
  ,hub
  ,market
  ,lmp_total
  ,lmp_system_energy_price
  ,lmp_congestion_price
  ,lmp_marginal_loss_price
from pjm_cleaned.pjm_lmps_hourly
```

When adding a new mart model, create a matching `.sql` file in the domain's `.docs/` folder.

#### Keeping Docs in Sync with Mart Changes

When a mart view is updated (columns added, removed, or renamed), you **must** update all related documentation:

1. **`.docs/<mart_name>.sql`** — update the SELECT column list to match the new view
2. **`.docs/README.md`** — update Key Columns, Transformation Notes, and any other affected sections
3. **`schema.yml`** — add/remove/rename column entries and descriptions
4. **`documentation/docs/domains/<domain>/dbt-views/`** — update the corresponding Docusaurus page (Business Definition, Key Columns, `**SQL**` GitHub link, etc.)
5. **`.sql_queries/`** — update any validation queries that reference changed columns

#### README.md

```markdown
# <domain>_cleaned marts

## Purpose
## Grain
## Source Relations
## Key Columns
## Transformation Notes
## Data Quality Checks
```

**Heading order is strict** — do not reorder or omit headings.

- **Purpose:** 1-2 sentences on what the mart exposes and who consumes it.
- **Grain:** Table with `Model` and `Grain` columns listing every mart model's primary key.
- **Source Relations:** Table with `Source` and `Upstream Staging Model` columns.
- **Key Columns:** Table with `Column` and `Description` columns for the most important output columns.
- **Transformation Notes:** Bullet list covering materialization strategy, where business logic lives, and domain-specific details.
- **Data Quality Checks:** Bullet list of `not_null`, `accepted_values`, and other schema tests from `schema.yml`.

### Current Mart Directories

| Mart | Marts Path | Docs Path |
|------|-----------|-----------|
| eia_cleaned | `models/eia/eia_cleaned/marts/` | `models/eia/eia_cleaned/.docs/` |
| pjm_cleaned | `models/power/pjm_cleaned/marts/` | `models/power/pjm_cleaned/.docs/` |
| meteologica_cleaned | `models/meteologica/meteologica_cleaned/marts/` | `models/meteologica/meteologica_cleaned/.docs/` |
| genscape_cleaned | `models/genscape/genscape_cleaned/marts/` | `models/genscape/genscape_cleaned/.docs/` |
| positions_cleaned | `models/positions_and_trades/positions_cleaned/marts/` | `models/positions_and_trades/positions_cleaned/.docs/` |
| trades_cleaned | `models/positions_and_trades/trades_cleaned/marts/` | `models/positions_and_trades/trades_cleaned/.docs/` |
| wsi_cleaned | `models/wsi/wsi_cleaned/marts/` | `models/wsi/wsi_cleaned/.docs/` |

## Adding a New Domain

1. Create `documentation/docs/domains/<domain>/overview.md`
2. Create `documentation/docs/domains/<domain>/scrapes/` and `documentation/docs/domains/<domain>/dbt-views/` as needed
3. Add the domain as a category in `documentation/sidebars.js` with doc IDs
4. Add entries to `documentation/docs/dbt-cleaned-catalog.md`
5. Update `documentation/docs/executive-summary.md` tables
6. Create `models/<domain>/<domain>_cleaned/.docs/` with a `README.md` and one `.sql` file per mart view

## Adding a New Scrape Page

1. Create the scrape card in `documentation/docs/domains/<domain>/scrapes/<scrape-name>.md`
2. Add its doc ID to the domain's Scrapes category in `documentation/sidebars.js`
3. Link from the domain overview's scrape inventory table
4. Link from the Data Catalog if it feeds a dbt view

## Deployment

- Pushes to `main` that touch `documentation/**` trigger the GitHub Actions workflow
- Workflow: install deps, `npm run build`, deploy to GitHub Pages
- Site URL: `https://akeavenyhelioscta.github.io/helioscta-backend/`
- Requires a `github-pages` environment in repo Settings > Environments

## QA Checklist

- Dark mode: true black background, high-contrast text, amber active nav
- Light mode: toggle works, no regressions
- Mobile: sidebar collapses, tables scroll horizontally
- Links: `onBrokenLinks: "throw"` catches broken links at build time
- Search: Docusaurus built-in search works out of the box
- Admonitions: use `:::` syntax, not `!!!`
