...
export default function HomePageClient() {
  ...
  // Re-fetch watchlists on mount and whenever user navigates to the watchlists tab
  // (so newly created/updated watchlists appear without a full page refresh)
  useEffect(() => {
    if (!GENSCAPE_ENABLED) {
      setWatchlistsLoading(false);
      return;
    }
    if (activeSection !== "home" && activeSection !== "watchlists") return;
    ...
    fetch("/api/watchlists")
      ...
      .then((data) => {
        ...
        setActiveWatchlist((prev) => {
          if (prev && wls.find((w) => w.id === prev.id)) return prev;
