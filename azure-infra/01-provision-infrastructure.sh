...
#   4) Test Azure PostgreSQL Flexible Server + database
#
# Prerequisites:
#   - Azure CLI installed and authenticated (az login)
#   - config.sh populated (subscription, location, test DB names)
#
# Usage:
...
echo ""
if az postgres flexible-server show --resource-group "$RG" --name "$TEST_PG_SERVER" --output none 2>/dev/null; then
  echo "=== Test PostgreSQL Server '$TEST_PG_SERVER' already exists - skipping ==="
else
  echo "=== Creating Test PostgreSQL Server: $TEST_PG_SERVER ==="

  if [[ -z "${TEST_PG_PASSWORD:-}" ]]; then
    echo -n "Enter password for test PostgreSQL admin ($TEST_PG_ADMIN): "
    read -rs TEST_PG_PASSWORD
    echo ""
  fi
  ...
  az postgres flexible-server create \
    ...
    --name "$TEST_PG_SERVER" \
    --location "$LOCATION" \
    --admin-user "$TEST_PG_ADMIN" \
    --admin-password "$TEST_PG_PASSWORD" \
...
echo "=== Ensuring Test Database exists: $TEST_PG_DB ==="
az postgres flexible-server db create \
  --resource-group "$RG" \
  --server-name "$TEST_PG_SERVER" \
  --database-name "$TEST_PG_DB" \
...
TEST_HOST="${TEST_PG_SERVER}.postgres.database.azure.com"
...
echo "  Test PostgreSQL Server: $TEST_PG_SERVER"
echo "  Test PostgreSQL DB:     $TEST_PG_DB"
echo "  Host:                   $TEST_HOST"
echo ""
