"""PreToolUse hook: ensure the backend-views MCP server is healthy.

Fires before any ``mcp__backend-views__*`` tool call (matcher scoped in
.claude/settings.json). Fast localhost health check; only restarts when
the server is unreachable, so the steady-state cost is one quick HTTP
GET (~10-50ms on a warm server).

Reads the standard Claude Code hook JSON payload from stdin but doesn't
need to inspect it — the matcher already filtered us to the right tool
calls.

Exit codes:
    0  = MCP healthy (no-op, or restart succeeded). Tool call continues.
    2  = MCP unhealthy and restart failed. Tool call blocked; stderr is
         shown to the model so it can surface the failure cleanly.
"""
